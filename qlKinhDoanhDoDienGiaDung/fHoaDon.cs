...
namespace qlKinhDoanhDoDienGiaDung
{
    public partial class frmHoaDon : Form
    {
        ...
        {
            string sql;
            sql = "SELECT a.MaSP, b.TenSP, a.SoLuong, b.DonGiaBan, a.GiamGia, a.ThanhTien FROM CTHoaDon AS a, SanPham AS b WHERE a.MaHD = N'" + txtMaHoaDon.Text.Trim() + "' AND a.MaSP = b.MaSP";
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            sql = "SELECT MaSP FROM CTHoaDon WHERE MaSP=N'" + cboMaSanPham.SelectedValue + "' AND MaHD = N'" + txtMaHoaDon.Text.Trim() + "'";
            if (Functions.CheckKey(sql))
            {
            ...
            sql = "INSERT INTO CTHoaDon VALUES (N'" + txtMaHoaDon.Text.Trim() + "', N'" + cboMaSanPham.SelectedValue + "'," + txtSoLuong.Text.Trim() + ", " + txtDonGia.Text.Trim() + ", " + txtGiamGia.Text.Trim() + ", " + txtThanhTien.Text.Trim() + ")";
            Functions.RunSQL(sql);
            LoadDataGridView();
        ...
        private void dgvHoaDon_DoubleClick(object sender, EventArgs e)
        {
            ...
            if ((MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                ...
                sql = "DELETE CTHoaDon WHERE MaHD = N'" + txtMaHoaDon.Text + "' AND MaSP = N'" + MaHangxoa + "'";
                Functions.RunSQL(sql);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                //Xóa chi tiết hóa đơn
                sql = "SELECT * FROM CTHoaDon WHERE MaHD = N'" + txtMaHoaDon.Text + "'";
                DataTable dtRows = Functions.GetDataToTable(sql);
                for (int row = 0; row <= dtRows.Rows.Count - 1; row++)
                {
                    // Duyệt từng hàng để xóa
                    sql = "DELETE CTHoaDon WHERE MaHD = N'" + txtMaHoaDon.Text + "' AND MaSP = N'" + dtRows.Rows[row]["MaSP"] + "'";
